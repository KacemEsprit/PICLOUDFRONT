...
export class GeminiService {
  ...
  private buildClauses(partnerName: string, partnerSector: string, orgName: string, orgType: string, contractType: string, durationMonths: number): any[] {
    const clauses = [
      ...
      { numero: 2, titre: "Durée et Renouvellement",
      ...
      { numero: 3, titre: "Obligations des Parties",
      ...
      { numero: 4, titre: "Conditions Financières et Facturation",
      ...
      { numero: 5, titre: "Résolution des Litiges et Droit Applicable",
    ...
    if (partnerSector === 'TECHNOLOGY' || partnerSector === 'FINANCIAL') {
      clauses.push({
        ...
        contenu: partnerSector === 'TECHNOLOGY'
          ? `${partnerName} s'engage à intégrer ses solutions technologiques avec le système d'information de ${orgName} dans le respect des normes de sécurité ISO 27001. Les données des usagers du transport public collectées dans le cadre de ce partenariat seront traitées conformément à la loi tunisienne sur la protection des données personnelles.`
          : `${partnerName} s'engage à faciliter les transactions financières liées aux services de ${orgName} dans le respect des réglementations de la Banque Centrale de Tunisie. Les modalités de commission, les délais de réglement et les garanties bancaires seront définies en conformité avec la circulaire BCT applicable aux partenariats de transport public.`
      });
    }
